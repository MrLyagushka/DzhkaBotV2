...
async def do_test2(callback: CallbackQuery, state: FSMContext):
    ...
    choice1 = (await state.get_data())['choice1'].split('_')[3]
    
    task = Task()
    task.get_task(id_student=callback.message.chat.id)
    task_student = task.task_student

    await callback.message.answer(task_student[4], reply_markup=global_menu_student.markup)

    await callback.message.answer("Выбери правильный ответ..\n Додееелаааай")